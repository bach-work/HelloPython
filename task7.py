...
# должен создаваться объект-генератор. Функция вызывается следующим образом: for el in fact(n).
...
def fact(n: int):
    """Делает генератор с факториалами числа"""
    for i in range(1, n + 1):
        yield reduce(lambda prev, curr: prev * curr, range(1, i+1))
